...
推理“读取了 \cs{DoAssign}，然后展开，空格分隔了数字 800，因此分配了 800，并打印了零”是不正确的。请注意，如果零出现在下一行上，情况也是一样的。
...
输出结果为\begin{disp} a b'\end{disp}，因为在 \cs{relax} 控制序列后的空格只有在首次读取该行时才会被跳过，而不是在展开时。另一方面，片段：
