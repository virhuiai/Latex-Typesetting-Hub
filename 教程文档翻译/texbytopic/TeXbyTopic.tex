...
% \setcounter{chapter}{1} \input{ch02/ch02.tex}
...
因此，在这里，\TeX\ 的行为不依赖于操作系统及其使用的行终止符（\key{CR}-\key{LF}、\key{LF} 或没有终止符用于块存储）。
从输入行中删除任何尾随的空格。
这样做的原因是历史原因；它与 \key{IBM} 大型计算机上的块存储模式有关。
...
然后，追加一个字符代码为 \cs{endlinechar} 的终止符字符，
除非此参数的值为负数或大于 255。
...
在这个定义中，输入中的空行不会被考虑在内：它们会消失，因为两个连续的 \cs{par} 记号（在此情况下）等效于一个。对于行尾的稍作修改的定义如下：
\begin{verbatim}
