...
字符序列的连写替换受控于字体的 \n{tfm} 文件中的信息。在某些字体中，\gr{character} 命令可以形成连写：例如，\n{fi} 这样的序列在某些字体中会被替换为 `fi'。
...
\TeX{} 中的连写可以在显式字符记号、\cs{char} 命令和 \gr{chardef token} 之间形成。例如，如果字体中包含 fi' 连写，序列 \verb-\charf\char`i- 会被替换为连写。
...
每个单词都由左边界字符和右边界字符包围（仅限于 \TeX3）。这使得希腊语中的两个不同的 sigma 成为可能：一个用于单词的末尾，另一个用于其他位置。这可以通过与边界字符进行连写来实现。在单词之前或之后立即使用 \csidx{noboundary} 命令可以取消该位置处的边界字符。
