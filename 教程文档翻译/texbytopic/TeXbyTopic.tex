...
\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_p.tex}
%xelatexdef --tempdir /Volumes/RamDisk --texoptions '-shell-escape' -c '[twoside,letterpaper]{rapport3}' -o '\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_p.tex}' 
\usepackage[all]{tcolorbox}
\newtcolorbox{引述之言}[1][virhuiai]{%
...
% \awp=\relax.
\begin{document}
\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_before_text.tex}
% xelatexdef --tempdir /Volumes/RamDisk --texoptions '-shell-escape' -c '[twoside,letterpaper]{rapport3}' -o '\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_p.tex}' -b '\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_before_text.tex}' -s

% \input{TeXbyTopic_000.tex}%封面
% % \input{TeXbyTopic_License.tex}%先不翻译
% \input{ch01/ch01.tex}%v1 2023 10 23
% \setcounter{chapter}{1} \input{ch02/ch02.tex}%类别码和内部状态
% \setcounter{chapter}{2} \input{ch03/ch03.tex}%-ch03-字符
% \setcounter{chapter}{3} \input{ch04/ch04.tex}%-ch04-字体
% \setcounter{chapter}{4} \input{ch05/ch05.tex}%-ch05-盒子
% \setcounter{chapter}{5} \input{ch06/ch06.tex}%ch06-水平模式和垂直模式
\setcounter{chapter}{6} \input{ch07/ch07.tex}%ch07-数字



%\spoint Floating-point arithmetic
\subsection{Floating-point arithmetic\\浮点数运算}
...
在内部，一些浮点数的算术运算（即在粘连比例的计算中）被执行。
然而，舍入的机器相关方面不能影响 \TeX\ 的决策过程，因此在这方面保证了 \TeX\ 的机器无关性
（舍入的足够准确性由~\cite{K:trip} 的 \n{Trip} 测试保证）。
%\spoint Fixed-point arithmetic
\subsection{Fixed-point arithmetic\\定点数运算}
...
\TeX\ 中的所有分数运算都使用固定点的“缩放整数”进行
\term arithmetic! fixed-point\par
固定点算术：$2^{-16}$ 的倍数。
这确保了 \TeX\ 的机器无关性。
缩放整数的打印表示将四舍五入到小数点后五位。
...
在常规的 32 位 \TeX\ 实现中，最大的整数绝对值为 $2^{31}-1$。
用户不被允许指定大于 $2^{30}-1$ 绝对大小的尺寸：
在 32 位系统上，两个这样的尺寸可以相加或相减而不会溢出。

%\point Number testing
\section{Number testing\\数字测试}
...
其中 \gr{relation} 是一个类别码为 12 的 \n<、\n> 或 \n= 符号。

Distinguishing between odd and even numbers is done
by 

区分奇数和偶数可以通过\begin{disp}\cs{ifodd}\gr{number}\end{disp}
...
其中 \cs{else} 部分是可选的。如果数字是 $i$，则会处理 \gr{case$_i$} 的记号；其他情况将被跳过，类似于条件语句中通常发生的情况（参见第~\ref{if} 章）。

%\point Remarks
\section{Remarks\\备注}

%\spoint Character constants
\subsection{Character constants\\字符常量}
...
在格式和宏集中经常需要使用数字常量。在 \TeX 中，有几种实现这些常量的方法。
...
这种方法更好，但容易出错：\TeX 需要展开才能找到数字 \ldots 这本身就是一种轻微的开销 \rdash 并且可能无意中展开一些本应保持不变的记号。
...
这种方法是好的。一个 \gr{chardef token} 与一个 \cs{count} 寄存器具有相同的状态：它们都是 \gr{internal integer}。因此，以这种方式定义的数字可以在任何适用 \cs{count} 寄存器的地方使用。对于较大的数字，可以用 \cs{mathchardef} 替换 \cs{chardef}，其范围为 \verb>"7FFF>${}=32,767$。请注意，\gr{mathchardef token} 通常只能出现在数学模式中，但在数字的上下文中可以出现在任何地方。

%\spoint Expanding too far / how far
\subsection{Expanding too far / how far\\扩展过远/多远}
...
在编写 \TeX\ 代码时，常见的错误是 \TeX\ 会无意中展开某些内容，因为它试图组成一个数字。
例如：
\begin{verbatim}
...
这里页码不会被错误地吸收：
\TeX\ 在 \cs{penalty} 之后试图找到一个 \gr{number}，
并找到了一个 \gr{countdef token}。这个 \gr{countdef token} {\em 不会}
转换为数字表示，因此页码永远不会被修改。

It is possible to convert a \gr{countdef token} first to
a representation in digits before assigning it:

可以先将 \gr{countdef token} 转换为数字表示再进行赋值：
\begin{verbatim}
\penalty\number\midpenalty
\end{verbatim}
...
xelatexdef --tempdir /Volumes/RamDisk --texoptions '-shell-escape' -c '[twoside,letterpaper]{rapport3}' 
xelatexdef --tempdir /Volumes/RamDisk --texoptions '-shell-escape' -c '[twoside,letterpaper]{rapport3}' -o '\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_p.tex}' -b '\input{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/教程文档翻译/texbytopic/TeXbyTopic_before_text.tex}' -f -s