...
在 plain \TeX\ 中，宏 \csidx{removelastskip} 的定义如下：\begin{verbatim}
...
有时候，我们需要智能化的命令，例如 \cs{vskip}，即连续的两个间距命令应该只留下较大的那个间距。在第~\pageref{skip:scheme}~页使用了这样的宏：
\begin{verbatim}
\newskip\tempskipa
...
首先，这个宏会检测模式是否为垂直模式；如果不是，参数可以安全地放置。将参数复制到一个间距寄存器中是必要的，因为在 \verb>\ifdim#1<\lastskip> 测试中，\cs{vspace}\verb>{2pt plus 3pt}> 会导致问题。
...
不会放置任何粘连，因为在 \cs{par} 之后，我们处于垂直模式，并且 \cs{lastskip} 的值为 \n{0pt}。
...
如果通过将 \cs{tracingoutput} 设置为正值来跟踪 \TeX\ 的工作过程，或者如果 \TeX\ 将一个盒子写入日志文件（因为有一个 \cs{showbox} 命令，或者因为它超过或不足），粘连将由控制序列 \cs{glue} 表示。这不是一个 \TeX\ 命令；它仅仅表示当前列表中存在粘连。
...
\TeX 生成的盒子表示，例如通过 \cs{showbox}，在每个显式的 \cs{kern} 后面插入一个空格，但在字体的字符间距信息中插入的隐式 kern 后面不插入空格。因此，\hbox{\verb-\kern 2.0pt-} 表示由用户或宏插入的 kern，而 \verb-\kern2.0pt- 表示隐式 kern。

Glue that is inserted automatically (\cs{topskip}, \cs{baselineskip},
et cetera) is denoted by name in \TeX's trace output.
For example, the box

在 \TeX 的跟踪输出中，自动插入的粘连（\cs{topskip}、\cs{baselineskip} 等）以名称表示。例如，盒子
\begin{verbatim}
\vbox{\hbox{Vo}\hbox{b}}
\end{verbatim}
looks like
