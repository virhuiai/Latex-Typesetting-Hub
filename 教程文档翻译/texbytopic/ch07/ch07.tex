...
% \section{Integer registers\\整数寄存器}

% Integers can be stored in \csidx{count} registers:

% 整数可以存储在 \csidx{count} 寄存器中：
...
% 是一个\gr{整数变量}和一个\gr{内部整数}。
% 作为整数变量，它可以在\gr{变量赋值}中使用：
...
% 可以使用 \csidx{countdef} 命令在\gr{速记定义}中引入 \cs{count} 寄存器的同义词：
...
% 以这种方式定义的控制序列称为\gr{countdef token}，它作为一个\gr{内部整数}。
...
% Plain \TeX\ 的宏 \csidx{newcount}
% （声明为 \cs{outer}）使用 \cs{countdef} 命令来分配一个未使用的 \cs{count} 寄存器。
% 计数器 0--9 是临时寄存器，就像所有编号为 0--9 的寄存器一样。
% 然而，计数器 0--9 在 \n{dvi} 文件中用于页面标识（请参见第~\ref{TeXcomm} 章），
% 因此它们应仅在组内作为临时寄存器使用。
% 计数器 10--22 用于 plain \TeX\ 对寄存器分配的记录。
% 计数器 255 也是临时寄存器。



% %\point Arithmetic
% \section{Arithmetic\\算术运算}
...
% 用户可以在 \TeX\ 中进行一些算术运算，而 \TeX\ 也可以在内部执行算术运算。
% 用户算术运算仅涉及整数；内部算术运算主要涉及定点数，仅在设置粘连时涉及浮点数。
% %\spoint Arithmetic statements
% \subsection{Arithmetic statements\\算术语句}
...
% 整数的除法得到的是一个整数；也就是说，忽略了余数。这引发了一个问题：当操作数中的一个为负数时，如何进行舍入。在这种情况下，\TeX\ 使用操作数的绝对值来执行除法，并且如果只有一个操作数为负数，则取结果的负数。
