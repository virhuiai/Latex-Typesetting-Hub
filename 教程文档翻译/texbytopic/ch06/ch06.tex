...
三个条件语句 \cs{ifhmode}、\cs{ifvmode} 和 \cs{ifinner} 可以区分 \TeX\ 的四种非数学模式。
如果 \TeX\ 处于受限水平模式、内部垂直模式（或非陈列数学模式）中，则 \cs{ifinner} 测试为真。
特殊情况：在 \cs{write} 过程中，\TeX\ 处于“无模式”状态。
此时，\cs{ifhmode}、\cs{ifvmode} 和 \cs{ifmmode} 的测试结果均为假。
...
通过命令 \csidx{showlists}\label{showlists}，可以检查所有当前列表，包括“最近贡献”（请参见第~\ref{page:break} 章）。
...
请注意，无限粘连和\cs{break}惩罚仍然是水平列表的一部分。
