...
% \noindent
% 这个 Perl 脚本，使用和 \TeX\ 原始命令 \cs{show} 类似的方式，展示 \LaTeX\ 命令的定义。
% 它旨在让用户能快速查看到命令的定义以及registers的值，不论是用户自定义的，还是宏包包含。
% 指定的命令 are compiled internally with \LaTeX\ and 结果 of \cs{show} 被格式化并展示给用户。%v1

% 这个 Perl 脚本以与 \TeX\ 原始命令 \cs{show} 类似的方式显示 \LaTeX\ 命令的定义。
% 它旨在允许用户快速查看用户级别或内部包宏的定义以及寄存器的值。给定的命令在 \LaTeX\ 中被内部编译，
% \cs{show} 的输出被格式化并显示给用户。%v2
\noindent
...
% \begin{docCommand}{texdef}{~\oarg{options}~ \meta{commandname} \colOpt{\ttfamily [ \meta{commandname} ... ]}}
% \end{docCommand}

% \begin{docCommand}{latexdef}{~\oarg{options}~ \meta{commandname} \colOpt{\ttfamily [ \meta{commandname} ... ]}}
% \end{docCommand}

texdef ~\oarg{options}~ \meta{commandname} \colOpt{\ttfamily [ \meta{commandname} ... ]}

latexdef ~\oarg{options}~ \meta{commandname} \colOpt{\ttfamily [ \meta{commandname} ... ]}

Other program names are possible. See the \texttt{tex} option.  Command names do not need to start with `\verb|\|'.

% 程序名称也可能其他名称，见 \texttt{tex} 选项的说明.  命令名称不需要用 `|\|' 开始。%v1
% 其他程序名称也是可能的。请参见 \texttt{tex} 选项。命令名称不需要以 `|\|' 开头。%v2
