...
or `context' is used. For LaTeX the \meta{pkg} can start with `[<options>]` and end
with \meta{pkgname} or \oarg{pkgname}.

根据使用的是 `*tex'，`*latex'还是`context'，加载给定的tex文件、包或模块。对于LaTeX，\meta{pkg}可以以`[<options>]'开头，并以\meta{pkgname} 或\oarg{pkgname} 结尾。
\cend
\end{options}
% --package \meta{pkg}, -p \meta{pkg}      \csep 加载指定的 tex-file, package 或 module， depending on whether `*tex', `*latex'
%                                                        or `context' is used. 对 LaTeX，\meta{pkg} 可以是带参数的 \MacroArgs[<参数>]\oarg{pkgname}，也可以是不带参数的 \oarg{pkgname}..\cend

\begin{options}
  --class \meta{class}, -c \meta{class}    \csep 
(LaTeX only) Load given class instead of default (`article').
The \meta{class} can start with \MacroArgs[<class options>] and end
with \meta{classname} or \oarg{classname}.
  
（仅限LaTeX）加载给定的类，而不是默认的（`article'）。\meta{class} 可以以\MacroArgs[<class options>] 开头，并以\meta{classname} 或\oarg{classname} 结尾。
  \cend
\end{options}
% --class \meta{class}, -c \meta{class}    \csep (仅LaTeX) 指定加载的文类(默认是`article').
% \meta{class} 可以是带参数的 \MacroArgs[<参数>]\oarg{classname}，也可以是不带参数的 \oarg{classname}.\cend

\begin{options}
  --environment \meta{env}, -p \meta{env}  \csep 
(M) Show definition inside the given environment \meta{env}.

（M）显示在给定的环境\meta{env} 中的定义。
\cend
\end{options}
% --environment \meta{env}, -e \meta{env}  \csep 展示命令在指定的环境中的定义 \meta{env}.\cend

\begin{options}
  --othercode \meta{code}, -o \meta{code}  \csep
...
（M）在显示定义之前，将其他代码添加到序言中。这可以用来例如加载PGF/TikZ库。
  \cend
\end{options}
% --othercode \meta{code}, -o \meta{code}  \csep 在展示命令的定义前，在导言区添加另外的代码。
