...
\refCom{tcbstartrecording} has to be called before; otherwise, \refCom{tcbrecord} is silently ignored.
\tcblower
将任何\meta{content}记录到记录文件中。%
\refCom{tcbrecord}实现为|\immediate\write|。%
必须先调用\refCom{tcbstartrecording};否则，\refCom{tcbrecord}将被默默地忽略。
\end{stripedbox}
...
\begin{stripedbox}
Closes the current record file which was opened by \refCom{tcbstartrecording} before.
\tcblower
关闭当前由\refCom{tcbstartrecording}打开的记录文件。
\end{stripedbox}
\end{docCommand}
...
\tcblower
将任何\meta{content}记录到记录文件中，参见\refCom{tcbrecord}。%
可以多次使用来写多行。
\end{stripedbox}
...
\begin{stripedbox}
Disables \refCom{tcbrecord} and \refKey{/tcb/record} inside the current   group.
\tcblower
在当前组中禁用\refCom{tcbrecord}和\refKey{/tcb/record}。
\end{stripedbox}
...
Each pair is generated inside a single |tcolorbox| where the solution is given below \refCom{tcblower}. 
For every example, the solution part is saved by \refKey{/tcb/savelowerto} to a file. 
The saving is recorded using \refKey{/tcb/record}. 
...
每对都在单个|tcolorbox|中生成，其中参考答案在\refCom{tcblower}下面给出。%
对于每个示例，参考答案部分都通过\refKey{/tcb/savelowerto}保存到一个文件中。%
保存记录使用\refKey{/tcb/record}\footnote{可以多次使用来写多行}。%
为了启发可能性，第二个练习没有解决方案。%
