...
%squeezed 挤压
\begin{docTcbKey}[][doc new=2014-11-24]{squeezed title}{=\meta{text}}{style, no default, initially unset,标题宽度限最大宽}
...
\begin{docTcbKey}{detach title}{}{no value}
  ...
  将标题从它的正常位置分离。标题的文本存储到 \docAuxCommand{tcbtitletext} 中，格式化的标题可以通过 \docAuxCommand{tcbtitle} 获得。主要的应用是将标题从它的惯例位置移动到另一个位置。
  ...
  这是一个\textbf{tcolorbox}。\footnote{译者idea:可以改造了当单条的description。}
  \end{mybox}
...
另见 \mylib{skins} 库的 \refKey{/tcb/colbacklower}。
...
根据给定的值切换标题部分背景色的绘制。
\refKey{/tcb/colbacktitle},\refKey{/tcb/opacitybacktitle}, \refKey{/tcb/title style},
和 \refKey{/tcb/title code} 设定时，会自动将此选项设置为 |true|。
...
\begin{docTcbKey}{fontupper}{=\meta{text}}{no default, initially empty}
  ...
  附加 \meta{text} 到upper部分的内容之前(e.\,g.\ 字体设置)。
...
\begin{docTcbKey}[][doc new=2015-05-07]{halign lower}{=\meta{alignment}}{no default, initially \texttt{justify}}
  ...
  |halign lower| 控制着盒子的lower部分的内容的水平对齐方式为 \meta{alignment}。\meta{alignment} 的可选值同 \refKey{/tcb/halign} 一样。
...
\begin{docTcbKey}[][doc new=2022-10-30]{halign title}{=\meta{alignment}}{no default, initially \texttt{justify}}
  ...
  \meta{alignment} 的可选值同 \refKey{/tcb/halign} 一样。
...
\begin{docTcbKey}[][doc updated=2015-05-07]{flushleft upper}{}{style, no value}
  ...
  将 \refKey{/tcb/halign} 设置为 \docValue{flush left} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign} 设置为 \docValue{flush center} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign} 设置为 \docValue{flush right} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign lower} 设置为 \docValue{flush left} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign lower} 设置为 \docValue{flush center} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign lower} 设置为 \docValue{flush right} 的简写形式。
  \end{docTcbKey}
...
\begin{docTcbKey}[][doc updated=2015-05-07]{flushleft title}{}{style, no value}
  ...
  将 \refKey{/tcb/halign title} 设置为 \docValue{flush left} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign title} 设置主 \docValue{flush center} 的简写形式。
  \end{docTcbKey}
  ...
  将 \refKey{/tcb/halign title} 设置为 \docValue{flush right} 的简写形式。
  \end{docTcbKey}
  ...
  类似于\docValue{scale}, 但缩放范围受限于 \refKey{/tcb/valign scale limit}.
    \end{itemize}
...
设置 \docValue{scale*} 在指定 \refKey{/tcb/valign} 和 \refKey{/tcb/valign lower} 时缩放的范围上限。注意，此值不会随着 \refKey{/tcb/reset} 而重置。所以，修改的话也会同时在嵌套的盒子中生效。
\end{docTcbKey}


另见 \refKey{/tcb/sidebyside align} 了解当设置上下部分为左右排布时的对齐选项。
