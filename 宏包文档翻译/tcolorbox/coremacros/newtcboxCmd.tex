...
类似于|\newcommand|,基于\refComLe{tcbox}创建一个新的宏\texttt{\textbackslash}\meta{name}。  新的宏\texttt{\textbackslash}\meta{name}可选地接受\meta{number}$+1$个参数，其中 \meta{default}是可选第一个参数的默认值。 \meta{options}应用于底层的|tcbox|。 \meta{init options}允许设置自动编号， 参见第\pageref{sec:initkeys}页的第\ref{sec:initkeys}节。
