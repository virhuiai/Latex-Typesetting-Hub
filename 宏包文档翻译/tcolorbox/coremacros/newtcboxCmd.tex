...
% 类似于|\newcommand|,
基于\refComLe{tcbox}创建一个新的宏\texttt{\textbackslash}\meta{name}。  新的宏\texttt{\textbackslash}\meta{name}可选地接受\meta{number}个参数，其中 \meta{default}是可选第一个参数的默认值。 \meta{options}应用于底层的|tcbox|。 \meta{init options}可用于设置自动编号， 参见第\pageref{sec:initkeys}页的第\ref{sec:initkeys}节。
...
arc=7pt,
colback=#1!10!white,colframe=#1!50!black,
before upper={\rule[-3pt]{0pt}{10pt}},%简单实现增高
boxrule=1pt,
boxsep=0pt,
