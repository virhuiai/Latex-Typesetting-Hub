...
通常(但不一定)，|tcolorbox| 放置在单独的段落中，并且在它前后有一些垂直空间。这种行为由键 \refKeyLe{/tcb/before} 和 \refKeyLe{/tcb/after} 控制。
...
强制设置\refKeyLe{/tcb/nobeforeafter}，甚至\refKeyLe{/tcb/before}和\refKeyLe{/tcb/after}是在这个选项设置之后又设置的。不要全局使用此选项，除非您\emph{真}的知道在做什么。注意，嵌套的盒子不受影响。
\end{docTcbKey}
...
设置 \refKeyLe{/tcb/baseline} 的值.
可设置的 \meta{alignment} 值有:  
\begin{itemize}
