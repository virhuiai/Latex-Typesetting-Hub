...
对于浮动对象，\refKeyLe{/tcb/before}和\refKeyLe{/tcb/after}设置将被忽略。相反，给定的\meta{code}会在浮动框之前插入。如果框是\refKeyLe{/tcb/breakable}，则给定的\meta{code}会在分页盒子的每个部分之前插入。最常见的用法是|every float=\centering|。
