...
%   设置 \refKey{/tcb/arc} 为盒子内宽和内高中较小者值的一半。
  ...
  将\refKey{/tcb/arc}设置为盒子内宽度一半和高度一半的较小值。
...
%   这只适用于 \refKey{/tcb/height} 为固定值的情况。此外，\refKey{/tcb/bean arc} 选项设置需要在设置宽度和高度之后。
  这仅适用于固定的\refKey{/tcb/height}。另外，\refKey{/tcb/bean arc}必须在宽度和高度由选项键设置之后使用。
  \end{marker}
...
将\refKey{/tcb/arc}设置为盒子的内部宽度的$\frac{1}{2+\sqrt{2}}$。如果盒子的宽度和高度相同，则内部是一个正八边形。
...
此选项将重置 \refKey{/tcb/arc is angular} 所设置的修补程序。原始的 \tikzname\ 代码将被激活。
\end{docTcbKey}
...
设置\refKey{/tcb/left}，使得\meta{length}为左边界框和文本部分之间的距离。
...
将上部分的文本宽度设置为当前行宽再加上可选的\meta{length}。这是通过适当地更改键\refKey{/tcb/width}、\refKey{/tcb/enlarge left by}和\refKey{/tcb/enlarge right by}来实现的。结果的框重叠到页面的左右边距上。请注意，这个样式选项必须在所有其他几何键之后给出！还请参见\refKey{/tcb/grow sidewards by}和\refKey{/tcb/spread sidewards}。
