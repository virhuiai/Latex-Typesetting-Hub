...
\refEnvLe{tcolorbox} 的文本内容可以分为一个必需的\emph{上部分}和一个可选的\emph{下部分}。这些部分由\refComLe{tcblower}分隔。如果没有\refComLe{tcblower}，则没有\emph{下部分}，而\emph{上部分}构成完整的文本内容。
...
将框的内容保存到文件中，以备将来需要时使用。这是\refKeyLe{/tcb/savelowerto}的对应项，但不仅保存上部分，而是保存整个内容。如果存在下部分，则也会保存，包括\refComLe{tcblower}。
