...
将 |tcolorbox| 的图形环境设置为 \meta{name}。可行的值是 |pgfpicture| 和 |tikzpicture| 或继承自这两个环境的环境。该关键字由 \refKey{/tcb/skin} 设置，很少直接使用。
\end{stripedbox}
\end{docTcbKey}
...
由 \refKey{/tcb/frame engine} 绘制的盒子的 \emph{框架}。
\end{stripedbox}
...
盒子的 \emph{内部}。带标题的盒子的内部与不带标题的盒子不同。 使用 \refKey{/tcb/interior titled engine} 或 \refKey{/tcb/interior engine} 来绘制内部。
\end{stripedbox}
...
如果有下部，则绘制盒子的 \emph{分割}（线）；由 \refKey{/tcb/segmentation engine} 绘制。
\end{stripedbox}
...
如果有标题并且 \refKey{/tcb/title filled} 设置为 |true|，则绘制盒子的 \emph{标题区域}；由 \refKey{/tcb/title engine} 绘制。
\end{stripedbox}
\end{enumerate}
