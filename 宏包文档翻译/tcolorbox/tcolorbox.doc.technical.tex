...
本节提供了 |tcolorbox| 的皮肤概念的技术概述。对于大多数 |tcolorbox| 的应用，不需要了解此处描述的高级选项和设置。您可以继续阅读 \Vref{sec:skins}，其中记录了大多数用户的自定义选项。
\end{stripedbox}
...
% 请参阅\Vref{sec:skins}。
以下解释还涵盖了 \mylib{skins} 库的选项和设置，请参见 \Vref{sec:skins}。
\end{stripedbox}
...
从技术角度看，\emph{皮肤}是一个 |tcolorbox| 的外观样式定义。核心包提供了一些额外的皮肤选项键，但只有两个皮肤称为 \refSkin{standard} 和 \refSkin{standard jigsaw}。 \mylib{skins} 库添加了几个皮肤。要更改皮肤，只需设置核心包中的一个选项即可。
\end{stripedbox}
...
将当前的皮肤设置为 \meta{name}。这是一个样式定义，它设置了所有以下键，即对于许多用例，没有更多的事情要做。
\end{stripedbox}
...
Typically, this key is set by a \refKeyLe{/tcb/skin}.
\tcblower
如果盒子被设置为\refKeyLe{/tcb/breakable} 且实际上也是分页了,%
然后break序列的\emph{第一}部分的皮肤被设置为\meta{name}，
参见 \pageref{subsec:breaksequence} 页上的 \ref{subsec:breaksequence} 小节。
通常，该键由\refKeyLe{/tcb/skin}设置。
\end{stripedbox}
\end{docTcbKey}
...
This key is set by a \refKeyLe{/tcb/skin} and may seldom be used directly.
\tcblower
% 将|tcolorbox|的图形环境设置为\meta{name}。%
% 可行值为|pgfpicture|和|tikzpicture|或继承这两者之一的环境。%
% 该键由\refKeyLe{/tcb/skin}设置，可能很少直接使用。
将 |tcolorbox| 的图形环境设置为 \meta{name}。可行的值是 |pgfpicture| 和 |tikzpicture| 或继承自这两个环境的环境。该关键字由 \refKeyLe{/tcb/skin} 设置，很少直接使用。
\end{stripedbox}
\end{docTcbKey}
...
~\vspace*{-1em}
\begin{stripedbox}
The \emph{frame} of the box, drawn by \refKeyLe{/tcb/frame engine}.
\tcblower
% 盒子的\emph{frame}，由\refKeyLe{/tcb/frame engine}绘制。
由 \refKeyLe{/tcb/frame engine} 绘制的盒子的 \emph{框架}。
\end{stripedbox}
...
\refKeyLe{/tcb/interior titled engine} or \refKeyLe{/tcb/interior engine}   is used to draw the interior.
\tcblower
% 盒子的内部。
% 有标题的盒子内部与没有标题的盒子内部的绘制方式不同。
% \refKeyLe{/tcb/interior title engine}或\refKeyLe{/tcb/interior engine}用于绘制内部。
盒子的 \emph{内部}。带标题的盒子的内部与不带标题的盒子不同。 使用 \refKeyLe{/tcb/interior titled engine} 或 \refKeyLe{/tcb/interior engine} 来绘制内部。
\end{stripedbox}
...
drawn by \refKeyLe{/tcb/segmentation engine}.
\tcblower
% 盒子的\emph{segmentation}(line)，如果有lower的部分;
% 由\refKeyLe{/tcb/segmentation engine}绘制。
如果有下部，则绘制盒子的 \emph{分割}（线）；由 \refKeyLe{/tcb/segmentation engine} 绘制。
\end{stripedbox}
...
\refKeyLe{/tcb/title filled} is set to |true|; drawn
by \refKeyLe{/tcb/title engine}.
\tcblower
% 盒子中的\emph{title area}，
% 如果存在标题且 \refKeyLe{/tcb/title filled} 设置为 |true|; 使用 \refKeyLe{/tcb/title engine} 绘制。  
如果有标题并且 \refKeyLe{/tcb/title filled} 设置为 |true|，则绘制盒子的 \emph{标题区域}；由 \refKeyLe{/tcb/title engine} 绘制。
\end{stripedbox}
\end{enumerate}
...
\end{stripedbox}
  ...
  \item\docValue{path}: 
\begin{stripedbox}
a |tikz| path which is controlled by \refKeyLe{/tcb/frame style},
\tcblower
由\refKeyLe{/tcb/frame style}控制的|tikz|路径，
\end{stripedbox}
...
\end{stripedbox}  
  ...
  \begin{itemize}
  \item\docValue{standard}: the original code from the core package,
  \item\docValue{path}: a |tikz| path which is controlled by \refKeyLe{/tcb/interior style},
  \item\docValue{pathfirst}: a |tikz| path which is controlled by \refKeyLe{/tcb/interior style},
  \item\docValue{pathmiddle}: a |tikz| path which is controlled by \refKeyLe{/tcb/interior style},
  \item\docValue{pathlast}: a |tikz| path which is controlled by \refKeyLe{/tcb/interior style},
...
%  \begin{itemize}
%  \item\docValue{standard}: the original code from the core package,
%  \item\docValue{path}: a |tikz| path which is controlled by \refKeyLe{/tcb/segmentation style},
%  \item\docValue{freelance}: deprecated.
%  \item\docValue{spartan}: a quite spartan code.
%  \item\docValue{empty}: draw nothing.
%  \end{itemize}
...
%  \begin{itemize}
%  \item\docValue{standard}: the original code from the core package,
%  \item\docValue{path}: a |tikz| path which is controlled by \refKeyLe{/tcb/title style},
%  \item\docValue{pathfirst}: a |tikz| path which is controlled by \refKeyLe{/tcb/title style},
%  \item\docValue{pathmiddle}: a |tikz| path which is controlled by \refKeyLe{/tcb/title style},
%  \item\docValue{pathlast}: a |tikz| path which is controlled by \refKeyLe{/tcb/title style},
%  \item\docValue{freelance}: deprecated.
%  \item\docValue{spartan}: a quite spartan code.
%  \item\docValue{empty}: draw nothing.
%  \end{itemize}
...
%  Typically, this key is set automatically by a \refKeyLe{/tcb/skin}.
%\begin{dispExample}
%\tcbset{colback=Salmon!50!white,colframe=FireBrick!75!black,
...
%  %This option is available only if the \refKeyLe{/tcb/frame engine} is set to |freelance|.
%\begin{dispExample}
...
%  This is a shortcut for setting  \refKeyLe{/tcb/frame code} to empty.
%  This option removes the drawing of the frame.
%  Alternatively, use \refKeyLe{/tcb/frame hidden}.
%\end{docTcbKey}
%
%
...
%  %This option is available only if the \refKeyLe{/tcb/interior titled engine} is set to |freelance|.
%\begin{dispExample}
...
%  This is a shortcut for setting  \refKeyLe{/tcb/interior titled code} to empty.
%  This option removes the drawing of the untitled interior.
%  Alternatively, use \refKeyLe{/tcb/interior hidden}.
%\end{docTcbKey}
%
%
%
...
%  %This option is available only if the \refKeyLe{/tcb/interior engine} is set to |freelance|.
%\begin{dispExample}
%\tcbset{colback=red!5!white,colframe=red!75!black}
%
...
%  This is a shortcut for setting  \refKeyLe{/tcb/interior code} to empty.
...
%  %This option is available only if the \refKeyLe{/tcb/segmentation engine} is set to |freelance|.
%\begin{dispExample}
%\tcbset{colback=red!5!white,colframe=red!75!black,fonttitle=\bfseries}
%
...
%  This is a shortcut for setting  \refKeyLe{/tcb/segmentation code} to empty.
%  This option removes the drawing of the segmentation line.
%  Alternatively, use \refKeyLe{/tcb/segmentation hidden}.
%\end{docTcbKey}
%
%
%
...
%  %\\
%  %This option is available only if the \refKeyLe{/tcb/title engine} is set to |freelance|.
%\begin{dispExample}
%\tcbset{colback=red!5!white,colframe=red!75!black,fonttitle=\bfseries,
%  coltitle=black}
%
...
%  This is a shortcut for setting  \refKeyLe{/tcb/title code} to empty.
%  This option removes the drawing of the title area.
%  Alternatively, use \refKeyLe{/tcb/title hidden}.
%\end{docTcbKey}
%
%\clearpage
%\subsection{Subskins}\label{subsec:subskins}
%A subskin is a new \refKeyLe{/tcb/skin} based on an existing skin which
...
%the style \refKeyLe{/tcb/beamer} as pattern.
%\end{marker}
%
...
%  \refKeyLe{/tcb/skin},
%  \refKeyLe{/tcb/skin first},
%  \refKeyLe{/tcb/skin middle}, and
%  \refKeyLe{/tcb/skin last}.
...
%  existing \meta{base skin} plus the given \meta{options}.
%  This skin is set as \refKeyLe{/tcb/skin first}.\\
...
%  Creates a new unnamed skin which inherits all properties of an
%  existing \meta{base skin} plus the given \meta{options}.
%  This skin is set as \refKeyLe{/tcb/skin middle}.\\
%  See a detailed example on page~\pageref{freeboxexample}.
%\end{docTcbKey}
%
%
...
%  Creates a new unnamed skin which inherits all properties of an
%  existing \meta{base skin} plus the given \meta{options}.
%  This skin is set as \refKeyLe{/tcb/skin last}.\\
%  See a detailed example on page~\pageref{freeboxexample}.
%\end{docTcbKey}
%
%
...
%\end{testbox}
%\tcblower
%\item\refKeyLe{/tcb/colframe}, \refKeyLe{/tcb/opacityframe}
%\item\refKeyLe{/tcb/frame code}
%\item\refKeyLe{/tcb/frame style}
%\end{itembox}
%
%
%\begin{itembox}
...
%\end{testbox}
%\tcblower
%\item\refKeyLe{/tcb/colback}, \refKeyLe{/tcb/opacityback}
%\item\refKeyLe{/tcb/interior code}, \refKeyLe{/tcb/interior titled code}
%\item\refKeyLe{/tcb/interior style}
%\end{itembox}
%
%
%\begin{itembox}
...
%\end{testbox}
%\tcblower
%\item\refKeyLe{/tcb/colbacktitle}, \refKeyLe{/tcb/opacitybacktitle}
%\item\refKeyLe{/tcb/title code}
%\item\refKeyLe{/tcb/title style}
%\end{itembox}
%
%
%\begin{itembox}
...
%\end{testbox}
%\tcblower
%\item\refKeyLe{/tcb/lower separated}
%\item\refKeyLe{/tcb/segmentation code}
%\item\refKeyLe{/tcb/segmentation style}
%\end{itembox}
%
%
%\begin{itembox}
...
%\end{testbox}
%\tcblower
%\item\refKeyLe{/tcb/colupper}, \refKeyLe{/tcb/collower}, \refKeyLe{/tcb/coltitle}
%\item\refKeyLe{/tcb/fontupper}, \refKeyLe{/tcb/fontlower}, \refKeyLe{/tcb/fonttitle}
%\item\refKeyLe{/tcb/opacityupper}, \refKeyLe{/tcb/opacitylower}, \refKeyLe{/tcb/opacitytitle}
%\end{itembox}
%
%
%\begin{itembox}
...
%\begin{itemize}
%\item\docColor{tcbcolframe} set by \refKeyLe{/tcb/colframe} (frame color)
%\item\docColor{tcbcolback} set by \refKeyLe{/tcb/colback} (background color)
%\item\docColor{tcbcolbacktitle} set by \refKeyLe{/tcb/colbacktitle} (background color of the title)
%\item\docColor{tcbcolbacklower} set by \refKeyLe{/tcb/colbacklower} (skin dependend background color
%  of the lower part; needs \mylib{skins} to be loaded)
%\item\docColor{tcbcolupper} set by \refKeyLe{/tcb/colupper} (text color upper part)
%\item\docColor{tcbcollower} set by \refKeyLe{/tcb/collower} (text color lower part)
%\item\docColor{tcbcoltitle} set by \refKeyLe{/tcb/coltitle} (text color title)
%\end{itemize}
%
%\medskip
%
...
%  \item For \refKeyLe{/tcb/sidebyside} boxes, it describes the combined text
%    width plus segmentation.
%  \item This property can be used inside the box content text with exception
%    of \refKeyLe{/tcb/fit} boxes.
%  \item \refComLe{tcbtextwidth} can be used for all box types for skins or
%    inside \refKeyLe{/tcb/underlay}, \refKeyLe{/tcb/overlay}, etc.
%  \end{itemize}
%\begin{dispExample}
...
%  than \refComLe{tcbtextheight}.
%  \begin{itemize}
%  \item For boxes with a fixed \refKeyLe{/tcb/height}, this property can
%    be used inside the box content text. For other boxes, it denotes |0pt|
%    inside the box content.
%  \item \refComLe{tcbtextheight} can be used for all box types for skins or
%    inside \refKeyLe{/tcb/underlay}, \refKeyLe{/tcb/overlay}, etc.
%  \end{itemize}
%
%\begin{dispExample}
