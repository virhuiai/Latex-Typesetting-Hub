...
\begin{docCommand}[doc new and updated={2014-11-14}{2016-07-13}]{tcbincludegraphics}{\oarg{options}\marg{file name}}
  ...
  using the standard |\includegraphics| and puts it into a \refEnvLe{tcolorbox}.
...
\refComLe{tcbincludegraphics} to display the name of the file.
...
This is a generalized version of \refComLe{tcbincludegraphics} which allows
to include a complete PDF file denoted by \meta{file name}.
Every page is boxed into an own \refEnvLe{tcolorbox} customized by the
given \meta{options}. It is reasonable to put such a series of boxes
inside a \refEnvLe{tcbraster} for alignment.
...
\refComLe{tcbincludepdf} to display the current page number.
\end{docCommand}
...
Used for \refComLe{tcbincludegraphics} and \refComLe{tcbincludepdf} to
...
\refComLe{tcbincludepdf}.
\end{marker}
\end{docTcbKey}

\begin{docTcbKey}[][doc new=2014-11-14]{graphics pages}{=\meta{selection}}{no default, initially |1,...,|\cs{pdfpages}}
Used for \refComLe{tcbincludepdf} to specify a \meta{selection} of pages to be included.
