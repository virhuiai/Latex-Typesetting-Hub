...
The following options place the title text into an own \refComLe{tcbox}.
This boxed title can be customized independently from the main box using
\refKeyLe{/tcb/boxed title style}.
The placement can be influenced by \meta{boxtitle options}.

以下选项将标题文本放入自己的 \refComLe{tcbox} 中。 此盒装标题可以使用 \refKeyLe{/tcb/boxed title style} 独立自定义， 并且其位置可以受到 \meta{boxtitle options} 的影响。

\begin{docTcbKey}{attach boxed title to top left}{\colOpt{=\marg{boxtitle options}}}{style, default empty}
The title is boxed with a \refComLe{tcbox} and attached to
the top left corner of the main box.

标题用\refComLe{tcbox}盒子圈起来，附加在主盒子的左上角。
...
标题被用\refComLe{tcbox}框起来，附加在主框的左上角，并根据标题文本和框内文本进行调整。
...
标题用 \refComLe{tcbox} 包围，并附加在主框的右上角， 并移动以匹配标题文本和框文本。
...
标题使用\refComLe{tcbox}框起来，并附在主框的右上角。
...
标题用 \refComLe{tcbox} 框起来，附加在主框的左下角。
...
标题用 \refComLe{tcbox} 包围，并附加在主框的左下角，并移动以匹配标题文本和框文本。请注意，即使存在“lower部分”，它也与upper部分匹配。
...
标题被 \refComLe{tcbox} 包围，并附加在主框的右下角，移动以匹配标题文本和框文本。请注意，即使存在 \emph{下方} 部分，这也匹配 \emph{上方} 部分。
...
这个标题被 \refComLe{tcbox} 包围，并附加在主盒子的右下角。
...
\begin{docTcbKey}[][doc new=2016-02-26]{attach boxed title to top}{\colOpt{=\marg{boxtitle options}}}{style, default empty}
  ...
  It uses \refKeyLe{/tcb/attach boxed title to top center},
  \refKeyLe{/tcb/minipage boxed title}, and sizes the boxed title to match
  the base box.

这是一种方便的样式，可以模仿标准标题。 它使用\refKeyLe{/tcb/attach boxed title to top center}，\refKeyLe{/tcb/minipage boxed title}， 并调整标题框的大小以适应基础框。
...
\begin{docTcbKey}[][doc new=2016-02-26]{attach boxed title to top*}{\colOpt{=\marg{boxtitle options}}}{style, default empty}
  In contrast to \refKeyLe{/tcb/attach boxed title to top}, this style
...
与\refKeyLe{/tcb/attach boxed title to top}相比，这种样式使用较小的左右边框，以避免预览器出现故障。通常，不会像下面的示例一样为框架使用不同的颜色。
\begin{dispExample*}{sbs,lefthand ratio=0.66}
\begin{tcolorbox}[enhanced,title=My title,
  attach boxed title to top*,
  boxed title style={colframe=red}]
...
\begin{docTcbKey}[][doc new=2016-02-26]{attach boxed title to bottom}{\colOpt{=\marg{boxtitle options}}}{style, default empty}
  ...
  of the box.
  It uses \refKeyLe{/tcb/attach boxed title to bottom center},
  \refKeyLe{/tcb/minipage boxed title}, and sizes the boxed title to match
  the base box.

这是一种方便的样式，可以在盒子底部生成类似标准标题的效果。 它使用了\refKeyLe{/tcb/attach boxed title to bottom center}和\refKeyLe{/tcb/minipage boxed title}，并调整标题盒子的大小以与基本盒子匹配。
\begin{dispExample*}{sbs,lefthand ratio=0.66}
\begin{tcolorbox}[enhanced,title=My title,
...
与\refKeyLe{/tcb/attach boxed title to top}相比，此样式使用较小的左右边框以避免预览器故障。
\begin{dispExample*}{sbs,lefthand ratio=0.66}
\begin{tcolorbox}[enhanced,title=My title,
  attach boxed title to bottom*]
  This is a \textbf{tcolorbox}.
...
\begin{docTcbKey}[][doc new=2016-02-26]{flip title}{\colOpt{=\marg{options}}}{style, default empty}
  This style combines \refKeyLe{/tcb/attach boxed title to bottom*}
  with \refKeyLe{/tcb/boxed title style}. The \meta{options} are given to
  \refKeyLe{/tcb/boxed title style}.

这种样式结合了\refKeyLe{/tcb/attach boxed title to bottom*}和\refKeyLe{/tcb/boxed title style}。\meta{options}被赋予\refKeyLe{/tcb/boxed title style}。
\begin{dispExample*}{sbs,lefthand ratio=0.66}
