...
一个 \tikzname 节点 |title| 是由一个带框标题产生的，可以在 \refKey{/tcb/frame code}、\refKey{/tcb/interior code}、下层、上层和结束中使用。
\end{marker}
...
盒子标题几乎总是第一个底纹。唯一的例外是由\refKey{/tcb/underlay boxed title}定义的底纹，在此之前绘制。此外，由\refKey{/tcb/underlay boxed title}定义的底纹只有在实际设置了盒子标题时才绘制。如果没有盒子标题，它们将被忽略。
\end{marker}
...
默认情况下，带框标题的尺寸为 \refKey{/tcb/size}|=|\docValue*{title}，并继承主框的 \refKey{/tcb/skin} 和 \refKey{/tcb/colframe}。同时，\refKey{/tcb/colback} 也从主框的 \refKey{/tcb/colbacktitle} 继承。标题文本的字体和颜色设置与平常一样。所有其他的 \meta{options} 都由 \refKey{/tcb/boxed title style} 键设置。由于带框标题是通过 \refCom{tcbox} 设置的，因此所有 |tcolorbox| 选项均适用于此处。如果多次使用 \refKey{/tcb/boxed title style}，则会附加 \meta{options}。
...
删除所有由\refKey{/tcb/boxed title style}设置的选项。
\end{docTcbKey}
