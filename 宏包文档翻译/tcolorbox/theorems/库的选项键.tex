...
Sets \refKeyLe{/tcb/separator sign} to the default colon |:| sign.

将\refKeyLe{/tcb/separator sign}设置为默认的冒号|:|符号。
\end{docTcbKey}

\begin{docTcbKey}{separator sign dash}{}{style, no value}
Sets \refKeyLe{/tcb/separator sign} to an en-dash sign.

将\refKeyLe{/tcb/separator sign}设置为一个破折号符号。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Sets \refKeyLe{/tcb/separator sign} to empty.

将\refKeyLe{/tcb/separator sign}的集合设置为空。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Sets \refKeyLe{/tcb/description delimiters} to |(| and |)|.

将 \refKeyLe{/tcb/description delimiters} 设置为 |(| 和 |)|。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Sets \refKeyLe{/tcb/description delimiters} to the default empty texts.

将 \refKeyLe{/tcb/description delimiters} 设置为默认的空文本。
\end{docTcbKey}


\begin{docTcbKey}{description color}{\colOpt{=\meta{color}}}{default empty, initially empty}
Sets the \meta{color} of the descriptive title text deviating from \refKeyLe{/tcb/coltitle}.
The color is reset to \refKeyLe{/tcb/coltitle}, if |description color| is used without value.

设置与 \refKeyLe{/tcb/coltitle} 不同的描述标题文本的 \meta{color}。如果使用不带值的 |description color|，则颜色将重置为 \refKeyLe{/tcb/coltitle}。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Sets \meta{text} (e.\,g.\ font settings) before the descriptive title text deviating from \refKeyLe{/tcb/fonttitle}.
The \meta{text} is removed, if |description font| is used without value.

在与 \refKeyLe{/tcb/fonttitle} 不同的描述标题文本之前设置 \meta{text}（例如字体设置）。如果使用不带值的 |description font|，则会删除 \meta{text}。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Note that \refKeyLe{/tcb/description delimiters}, \refKeyLe{/tcb/description color},
and \refKeyLe{/tcb/description font} are ignored, if this option is used.\\
If |description formatter| is used without value, the formatter is reset
to its standard behavior.

将\meta{宏}设置为描述标题文本的格式化程序。 \meta{宏}必须接受一个必需参数（描述文本）。\ 请注意，如果使用此选项，则忽略\refKeyLe{/tcb/description delimiters}，\refKeyLe{/tcb/description color}和\refKeyLe{/tcb/description font}。\ 如果使用|description formatter|而没有值，则格式化程序将重置为其标准行为。
\begin{dispExample}
...
Sets \refKeyLe{/tcb/terminator sign} to the colon |:| sign.

将\refKeyLe{/tcb/terminator sign}设置为冒号|:|符号。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Sets \refKeyLe{/tcb/terminator sign} to an en-dash sign.

将\refKeyLe{/tcb/terminator sign}设置为短横线符号。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
Sets \refKeyLe{/tcb/terminator sign} to the default empty text.

将\refKeyLe{/tcb/terminator sign}设置为默认的空文本。
\end{docTcbKey}
...
are defined themselves by \refComLe{newtcbtheorem}. This \meta{separator} is
put between \meta{prefix} (defined by \refComLe{newtcbtheorem})
and \meta{marker} (defined by an actual theorem environment).

给定的 \meta{separator} 用于通过 \refComLe{newtcbtheorem} 定义的环境创建的标签。这个 \meta{separator} 被放置在 \meta{prefix}（由 \refComLe{newtcbtheorem} 定义）和 \meta{marker}（由实际的定理环境定义）之间。
\begin{dispExample}
\newtcbtheorem[use counter from=mytheo]{sometheorem}{定理}%
  {colback=white,colframe=red!50!black,fonttitle=\bfseries,
...
are defined themselves by \refComLe{newtcbtheorem}.
...
\meta{prefix} (defined by \refComLe{newtcbtheorem}),
\refKeyLe{/tcb/label separator},
and \meta{marker} (defined by an actual theorem environment).

所给出的 \meta{style} 用于与由 \refComLe{newtcbtheorem} 定义的环境创建的标签相关联。这个 \meta{style} 使用一个参数，该参数自动设置为环境的完整标签标记，即由 \refComLe{newtcbtheorem} 定义的 \meta{prefix}、\refKeyLe{/tcb/label separator} 和实际定理环境定义的 \meta{marker} 组成的文本。
\begin{dispExample}
...
A second usage of \refKeyLe{/tcb/theorem full label supplement} overwrites
the first setting.

对\refKeyLe{/tcb/theorem full label supplement}的第二次使用会覆盖第一次的设置。
\end{docTcbKey}
...
A second usage of \refKeyLe{/tcb/theorem label supplement} overwrites
the first setting, but
\refKeyLe{/tcb/theorem full label supplement}
and \refKeyLe{/tcb/theorem label supplement} can be used independently.

给定的\meta{style}与使用\refComLe{newtcbtheorem}定义的环境创建的标签相关联。 这个\meta{style}使用一个参数，该参数自动设置为实际定理环境定义的标签\meta{marker}。 第二次使用\refKeyLe{/tcb/theorem label supplement}会覆盖第一次设置，但\refKeyLe{/tcb/theorem full label supplement}和\refKeyLe{/tcb/theorem label supplement}可以独立使用。
\begin{dispExample}
...
created by hand or using \refComLe{newtcbtheorem}.

为了在文档内部具有常见的外观，关键字 |theorem| 不应直接使用，如上例所示，而应作为一个新环境的一部分手动创建或使用 \refComLe{newtcbtheorem}。
\end{docTcbKey}

% \clearpage

\begin{docTcbKey}{highlight math}{}{style, no value}
A style which is used for \refComLe{tcbhighmath} and which is
predefined as 

这是用于 \refComLe{tcbhighmath} 的样式，预定义为

|notitle,nophantom,colframe=red,colback=yellow!25!white|.

It can be changed with the usual |pgf| techniques or
with \refKeyLe{/tcb/highlight math style}.

可以使用常规的 |pgf| 技术或 \refKeyLe{/tcb/highlight math style} 来更改它。
\begin{dispExample}
\begin{align*}
  \tcbhighmath{1} + 1 &= 2,\\
...
Changes the definition for \refKeyLe{/tcb/highlight math} to
|notitle,nophantom| plus the given \meta{style definition}.
See \refComLe{tcbhighmath} for another example.

将 \refKeyLe{/tcb/highlight math} 的定义更改为 |notitle,nophantom| 加上给定的 \meta{样式定义}。另见 \refComLe{tcbhighmath} 的另一个示例。
\begin{dispExample}
% \tcbuselibrary{skins}
...
\refKeyLe{/tcb/ams equation} should / could not be used. Obviously, you are encouraged
to use \refKeyLe{/tcb/ams gather} in this case.

以下样式仅经过测试能够与原始的 |amsmath| 环境一起使用。如果例如 |equation| 环境被重新定义为 |gather|，那么 \refKeyLe{/tcb/ams equation} 就不能被使用，显然，在这种情况下，鼓励使用 \refKeyLe{/tcb/ams gather}。
\end{marker}
...
\refComLe{newtcolorbox}:

可以使用\refComLe{newtcolorbox}轻松创建新的彩色数学环境：
...
\bigskip
\begin{marker}
All described options like \refKeyLe{/tcb/ams gather upper}, \refKeyLe{/tcb/ams gather lower},
\refKeyLe{/tcb/ams gather} are (partially) setting (overwritting) the
keys \refKeyLe{/tcb/before upper}, \refKeyLe{/tcb/after upper},
\refKeyLe{/tcb/before lower}, \refKeyLe{/tcb/after lower}.\par
...
see \refKeyLe{/tcb/before upper app}.

所有描述的选项，如 \refKeyLe{/tcb/ams gather upper}、\refKeyLe{/tcb/ams gather lower}、\refKeyLe{/tcb/ams gather}，都（部分地）设置（覆盖）了键 \refKeyLe{/tcb/before upper}、\refKeyLe{/tcb/after upper}、\refKeyLe{/tcb/before lower}、\refKeyLe{/tcb/after lower}。\par 因此，例如 |\tcbset{ams gather,before upper={\text{Pythagoras:}}}| 会产生无效的结果。对于这种情况，建议使用\ |\tcbset{ams gather,before upper app={\text{Pythagoras:}}}|，参见 \refKeyLe{/tcb/before upper app}。
\end{marker}
...
\begin{marker}
The styles alter \refKeyLe{/tcb/separator sign}, \refKeyLe{/tcb/description delimiters},
\refKeyLe{/tcb/terminator sign}, and more. Therefore, one should apply such
keys \emph{after} a theorem style.

样式会改变 \refKeyLe{/tcb/separator sign}、\refKeyLe{/tcb/description delimiters}、\refKeyLe{/tcb/terminator sign} 等等。因此，在定理样式之后应该应用这些关键字。
\end{marker}
