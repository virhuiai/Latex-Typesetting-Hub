...
以下展示了使用\refCom{newtcbtheorem}突出数学定义、定理或类似内容的应用。
...
所有下列环境都使用一个公共计数器进行编号，但这可以很容易地更改。在这里，计数器输出还会加上子节编号。另外，使用 |cleveref| 宏包 \cite{cubitt:2018a} 进行智能引用。
...
通过 \refCom{newtcbtheorem}，现在可以创建常见编号的定理环境。|defstyle| 和 |theostyle| 用于外观样式。
...
请注意，上面的示例中使用了 \refKey{/tcb/label type} 来提供所需的名称信息，以便将其提供给 |cleveref|\cite{cubitt:2018a}。
\end{marker}
