...
记录一个带有给定的 \meta{name} 和可选的 \meta{key path} 的关键字。 给定的 \meta{options} 由 \refCom{tcbset} 设置。 该关键字以强制或可选的 \meta{parameters} 作为值， 并带有简短的 \meta{description}。 它会自动索引，并可以通过 \refCom{refKey}\marg{name} 引用。
\begin{dispExample}
