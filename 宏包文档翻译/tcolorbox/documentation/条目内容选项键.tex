...
command, environment, key, etc. For \refEnvLe{docCommand}, \refEnvLe{docEnvironment}, etc.
...
设置要记录的条目的\meta{name}，即命令、环境、键等的\meta{name}。对于\refEnvLe{docCommand}、\refEnvLe{docEnvironment}等，\meta{name}由必需参数设置，但也可以通过\refKeyLe{/tcb/doc name}设置。
...
设置文档条目的 \meta{参数}，即命令、环境、键等的 \meta{参数}。对于 \refEnvLe{docCommand}、\refEnvLe{docEnvironment} 等，\meta{参数}由必选选项设置，但也可以通过 \refKeyLe{/tcb/doc parameter} 设置。
\begin{dispExample}
\begin{docCommands}[
doc no index,  %  no index entries for this example
doc name      = textbf,
...
Sets the \meta{key path} of the key to document. For \refEnvLe{docKey}
and \refEnvLe{docKey*} the \meta{key path}  is set by a specialized option,
but can also be set by \refKeyLe{/tcb/doc keypath}.

将键的\meta{key path}设置为文档中的值。对于\refEnvLe{docKey}和\refEnvLe{docKey*}，\meta{key path}是通过专门的选项设置的，但也可以通过\refKeyLe{/tcb/doc keypath}设置。
\begin{dispExample}
\begin{docKeys}[
doc no index,  %  no index entries for this example
doc keypath     = tikz,
...
\refEnvLe{docCommand}, \refEnvLe{docEnvironment}, or \refEnvLe{docPathOperation}.
Such a description is
mandatory for \refEnvLe{docKey}.

为\refEnvLe{docCommand}、\refEnvLe{docEnvironment}或\refEnvLe{docPathOperation}设置一个（简短的！）附加的\meta{description}。对于\refEnvLe{docKey}，这样的描述是必需的。
\begin{dispExample}
\begin{docCommand*}[doc description=my description]{myCommandF}{\marg{argument}}
This is the documentation of \refComLe{myCommandF} which takes one \meta{argument}.
\refComLe{myCommandF} does some funny things with its \meta{argument}.

这是 \refComLe{myCommandF} 的文档，它接受一个 \meta{argument}。 \refComLe{myCommandF} 会对其 \meta{argument} 进行一些有趣的操作。
\end{docCommand*}
\end{dispExample}
\begin{marker}
...
If used inside the option list of \refEnvLe{docCommand}, \refEnvLe{docEnvironment},
\refEnvLe{docKey}, etc, then \meta{text} is used
for labeling instead of the name of the definition.

如果在 \refEnvLe{docCommand}、\refEnvLe{docEnvironment}、\refEnvLe{docKey} 等选项列表中使用，那么 \meta{text} 被用于标记，而不是定义的名称。
\begin{dispExample}
...
如果在\refEnvLe{docCommand}、\refEnvLe{docEnvironment}、\refEnvLe{docKey}等选项列表中使用，那么\meta{text}将用于索引而不是定义的名称。
\begin{dispExample}
...
如果在\refEnvLe{docCommand}、\refEnvLe{docEnvironment}、\refEnvLe{docKey}等选项列表中使用，那么\meta{text}将用作索引的排序关键字，而不是定义名称。
\begin{dispListing}
\begin{docCommands}[
...
environments. The same effect is achieved by using e.\,g.\ \refEnvLe{docCommand*}
instead of \refEnvLe{docCommand}.

如果设置为|false|，则不会为主要文档环境编写索引条目。使用例如\refEnvLe{docCommand*}而不是\refEnvLe{docCommand}可以达到相同的效果。
\end{docTcbKey}
...
Sets style \meta{options} for the displayed box of the \refComLe{tcbdocmarginnote} command.

为\refComLe{tcbdocmarginnote}命令的显示框设置样式选项\meta{options}。
\begin{dispExample}
...
\refEnvLe{docCommand}, \refEnvLe{docEnvironment}, etc.

在上方盒子的开头添加一个带有 \enquote{New: \meta{date}} 信息的边注。旨在用于 \refEnvLe{docCommand}、\refEnvLe{docEnvironment} 等选项列表内。 
