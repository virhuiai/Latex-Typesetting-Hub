...
记录一个带有给定\meta{name}的辅助或次要\LaTeX\ 宏，\meta{name}不带反斜杠。给定的\meta{options}由\refCom{tcbset}设置。此宏将自动索引为\refCom{docAuxCommand}，而\refCom{docAuxCommand*}没有索引条目。
\begin{dispExample}
...
记录一个带有给定\meta{name}的辅助或次要的\LaTeX 环境。 给定的\meta{options}使用\refCom{tcbset}设置。 此宏自动索引索引用于\refCom{docAuxEnvironment}，并且对于\refCom{docAuxEnvironment*}没有索引条目。
\begin{dispExample}
...
记录带有给定的 \meta{name} 和可选的 \meta{key path} 的辅助键。给定的 \meta{options} 通过 \refCom{tcbset} 设置。它会自动被 \refCom{docAuxKey} 索引，但不会有 \refCom{docAuxKey*} 的索引条目。
\begin{dispExample}
