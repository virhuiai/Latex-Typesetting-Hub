...
基于\refEnv{tcolorbox}创建一个带颜色的框。它将环境内容分别显示在框的上部源代码和下部编译文本中。外观由\refKeyLe{/tcb/documentation listing style} 和样式\refKeyLe{/tcb/docexample}控制。可以通过重新定义此样式来改变它。
{
...
\refEnv{dispExample} 的带星号版本接受 \refEnv{tcolorbox} 的 \meta{选项} 作为参数。这些 \meta{选项} 在 \refKeyLe{/tcb/docexample} 之后执行。
\begin{xdispExample}
