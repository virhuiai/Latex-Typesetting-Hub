...
从包|listings|\cite{hoffmann:listings}中设置选项。它们用于在\refEnv{dispExample}和\refEnv{dispListing}中排版代码清单。请注意，这与用于“常规”清单的键\refKeyLe{/tcb/listing options}不完全相同。仅用于\refKeyLe{/tcb/listing engine}|=listings|。
\end{docTcbKey}
...
设置在 \refEnv{dispExample} 和 \refEnv{dispListing} 之前执行的 \meta{macros}，除了 \refKeyLe{/tcb/before}。
\end{deprecated}
\end{docTcbKey}
...
设置在\refEnv{dispExample}和\refEnv{dispListing}之后执行的\meta{宏}，除了\refKeyLe{/tcb/after}之外的附加内容。
\end{deprecated}
\end{docTcbKey}
