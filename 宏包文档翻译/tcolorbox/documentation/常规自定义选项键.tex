...
使用颜色 |ExampleBack| 和 |ExampleFrame| 设置 \refEnv{dispExample} 和 \refEnv{dispListing} 的样式。要更改示例的外观，可以重新定义此样式。
\begin{dispListing}
% Predefined style:
\tcbset{
...
从包|listings|\cite{hoffmann:listings}中设置选项。它们用于在\refEnv{dispExample}和\refEnv{dispListing}中排版代码清单。请注意，这与用于“常规”清单的键\refKey{/tcb/listing options}不完全相同。仅用于\refKey{/tcb/listing engine}|=listings|。
\end{docTcbKey}
...
缩写为|documentation listing options={style=...}|。 此键设置|listings|包的\meta{style}，参见\cite{hoffmann:listings}。 请注意，这与用于“正常”列表的键\refKey{/tcb/listing style}不完全相同。\ 仅用于\refKey{/tcb/listing engine}|=listings|。
\end{docTcbKey}
...
如果使用，从包 |minted| \cite{poore:minted} 设置用于清单排版的选项。请注意，这与用于“常规”清单的键 \refKey{/tcb/minted options} 不完全相同。仅用于 \refKey{/tcb/listing engine}|=minted|。
\end{docTcbKey}
...
如果使用了 |minted| \cite{poore:minted} 包，则为其设置在 |Pygments| \cite{pygments:web} 中已知的 \meta{style}。请注意，这与用于“常规”列表的关键字 \refKey{/tcb/minted style} 不同。仅用于 \refKey{/tcb/listing engine}|=minted|。
\end{docTcbKey}
...
如果使用了包|minted|\cite{poore:minted}，则设置已知于|Pygments|\cite{pygments:web}的\meta{编程语言}。 请注意，这与用于“普通”列表的关键字\refKey{/tcb/minted language}不同。仅用于\refKey{/tcb/listing engine}|=minted|。
\end{docTcbKey}
...
以下两个键已被弃用且无功能（v3.50及以上版本）。 请使用适当的值替换\refKey{/tcb/before}和\refKey{/tcb/after}。 另请参阅\refKey{/tcb/docexample}。
\end{marker}
...
设置在 \refEnv{dispExample} 和 \refEnv{dispListing} 之前执行的 \meta{macros}，除了 \refKey{/tcb/before}。
\end{deprecated}
\end{docTcbKey}
...
设置在\refEnv{dispExample}和\refEnv{dispListing}之后执行的\meta{宏}，除了\refKey{/tcb/after}之外的附加内容。
\end{deprecated}
\end{docTcbKey}
...
在\refEnv{docEnvironment}，\refEnv{docCommand}等中使用的关键字以粗体（或不以粗体）打印。由于使用了打字机字体，因此在没有粗体变体的计算机现代字体或类似字体中效果可能看不见。请注意，对关键字的引用根本不以粗体打印。
...
将内部使用的\cs{index}宏替换为给定的\meta{宏}。 \meta{宏}必须像\cs{index}一样需要一个必选参数。 此选项与\refKey{/tcb/index command name}互斥。
\begin{dispListing}
...
将内部使用的\cs{index}宏替换为\mbox{\cs{index}\texttt{[\meta{name}]}}，即\mbox{\cs{index}\texttt{\textbraceleft\ldots\textbraceright}}被替换为\mbox{\cs{index}\texttt{[\meta{name}]\textbraceleft\ldots\textbraceright}}。此选项旨在与|imakeidx|一起使用，并且与\refKey{/tcb/index command}互斥。
\begin{dispListing}
...
设置 |makeindex| 的默认值，包括 \refKey{/tcb/index actual}、\refKey{/tcb/index quote} 和 \refKey{/tcb/index level}。
\end{docTcbKey}
...
如果设置为|true|，索引条目将用\refKey{/tcb/doclang/environment}、\refKey{/tcb/doclang/key}和其他短描述进行注释。
\end{docTcbKey}
...
如果设置为|true|，则索引条目将根据\refKey{/tcb/color environment}、\refKey{/tcb/color key}和其他颜色设置进行着色。
\end{docTcbKey}
...
设置 \refKey{/tcb/color command}、\refKey{/tcb/color environment}、\refKey{/tcb/color key}、\refKey{/tcb/color path}、\refKey{/tcb/color value}、\refKey{/tcb/color counter}、\refKey{/tcb/color length} 和 \refKey{/tcb/color color} 的高亮颜色。
\end{docTcbKey}
...
设置在\refEnv{docPathOperation} 中使用\colFade{\textbackslash path} 的淡化文本的颜色。
\end{docTcbKey}
