...
我认为这种类型的盒子也非常适用于突出显示定义和定理，因此我在我的数学讲义中也使用了它们%\cite{sturm:mathe1,sturm:mathe2,sturm:mathe3}
。
...
版本2.00的重要更新是添加了盒子的breakable支持(自动换页)。%
% 这个功能不太影响核心包。%不影响意思，去除
...
% 基础宏包|tcolorbox|加载了以下宏包：
% |pgf|%\cite{tantau:tikz_and_pgf}
% 、|verbatim|%\cite{schoepf:2001a}
% 、|etoolbox|%\cite{lehmann:etoolbox}
% 和|environ|%\cite{robertson:2014a}
% 。
% |tcolorbox|本身通常在导言区以常规方式加载：
...
% \begin{dispExample*}{sbs,title={上面的样式是这样实现的(有额外的设置)}}
% \begin{dispListing}
% \usepackage{tcolorbox}
% \end{dispListing}
% \end{dispExample*}
...
该包的选项采用|键-值|语法。例如%，用于排版代码列表的键是
：
%该包采用键值语法中的选项键。例如，设置列表的键是：
% \end{stripedbox}

% 该宏包使用键-值语法的选项键。
% 或者，您可以稍后在导言区使用这些键，使用\refCom{tcbuselibrary}（见该处）。
% 例如，排版列表的键是：
...
% \begin{stripedbox}
Alternatively, you may use these keys later in the preamble with \refComLe{tcbuselibrary} (see there).
% \tcblower
...
\begin{docCommand}{tcbuselibrary}{\marg{key list}}
加载由\makebox[0pt]{~}\meta{key list}\makebox[0pt]{~}给出的库。  \hfill Loads the libraries given by the \meta{key list}.
...
% 下面的键值
以下项（不含 |/tcb/library/|）可在|\tcbuselibrary|或|\usepackage|中使用。%%
% \end{stripedbox}
...
为盒子提供从一页自动断行到下一页的支持;参见\Fullref{sec:breakable}。
