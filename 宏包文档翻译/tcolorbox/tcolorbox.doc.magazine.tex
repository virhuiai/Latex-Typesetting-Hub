...
即使用了 \refKey{/tcb/reset and store to box array} 与新的盒子数组 |myarticle|，
...
使用 \refCom{consumetcboxarray} 将得到的部分分别分布到本章适当的位置。
像“在第 x 页继续”这样的链接文本是由嵌套的 \refCom{tcbox} 的 \refKey{/tcb/finish} 命令创建的。
使用 \refKey{/tcb/phantomlabel} 标记盒子部分。
...
在使用\refKey{/tcb/store to box array}之前，使用此命令或\refKey{/tcb/reset box array}。否则，所有的盒子都会被追加到已经存在的盒子中。
此命令不清除盒子寄存器。
\begin{dispListing}
...
重置盒子数组 \meta{name} 的大小计数器为零。如果未提供 \meta{name}，则默认使用 |default|。在应用 \refKey{/tcb/store to box array} 之前使用此命令或 \refCom{boxarrayreset}（执行相同操作）。否则，所有盒子都将附加到已存在的盒子上。该命令不清除盒子寄存器。
\begin{dispListing}
\tcbset{
...
该命令与\refCom{boxarrayreset}类似，将盒子阵列\meta{name}的大小计数器重置为零。此外，盒子阵列的所有已分配盒子寄存器都会清除其内容。请注意，分配的盒子寄存器仍保持分配状态。因此，这可能有助于清除内存，但不能释放盒子寄存器用于其他应用程序。如果使用\refCom{consumeboxarray}或\refCom{consumetcboxarray}应用存储的盒子，则使用\refCom{boxarrayclear}没有任何优势。
\begin{dispListing}
...
将 |tcolorbox| 或者 |tcolorbox| 的所有分段存储到盒子数组 \meta{name} 中。如果没有提供 \meta{name}，则使用已经存在的 |default| 盒子数组。否则，需要事先用 \refCom{newboxarray} 创建盒子数组。注意，如果要使盒子分成几个部分，则盒子必须是可分的（即具有 \refKey{/tcb/breakable} 选项）。通常情况下，手动断点需要通过 \refKey{/tcb/break at} 来添加。否则，盒子的每个部分将具有大约 |\textheight| 的长度。对于大多数用例，应该使用 \refKey{/tcb/reset box array} 来重置盒子数组计数器。
...
\useboxarray{1}\hfill
\begin{tabular}[b]{cc}
\multicolumn{2}{c}{\includegraphics[width=7cm]{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/宏包文档翻译/tcolorbox/Basilica_5.png}}\\
...
如果第一个盒子部分应该填充当前页面的所有剩余空间，您可以使用 $|\pagegoal-\pagetotal|$ 减去一些距离作为 \refKey{/tcb/break at} 的第一个元素。您可能需要在前面的文本中添加一些额外的距离。
\begin{dispListing}
% \usepackage{lipsum}
...
\includegraphics[width=\linewidth]{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/宏包文档翻译/tcolorbox/Basilica_5.png}\par
\lipsum[1-2]
\end{tcolorbox}
...
\refKey{/tcb/reset box array} 和 \refKey{/tcb/store to box array} 的组合。
\end{docTcbKey}
...
禁用 \refKey{/tcb/store to box array} 选项，如果之前已设置。
\end{docTcbKey}
...
将环境内容存储到一个名为 \meta{name} 的盒子数组中。这对应于标准的 \LaTeX\ 环境 |lrbox|，但存储操作是全局的。只要不使用 \refCom{boxarrayreset}，每个新的 \refEnv{boxarraystore} 都会向数组中添加一个新的盒子。
\begin{dispExample}
\boxarrayreset
...
从盒子数组\meta{name}中获取给定\meta{index}号的盒子并排版。如果没有给出\meta{name}，则使用已经存在的|default|盒子数组。如果使用不存在的盒子数组\meta{name}，则视为错误。如果\meta{index}超出范围，则静默忽略。请注意，\refCom{useboxarray}对应于标准的|\usebox|宏，即|\copy|。
\begin{dispExample}
\boxarraygetsize{\mysize}
...
从盒子数组\meta{name}中获取给定\meta{index}号的盒子并使用\refCom{useboxarray}作为\refCom{tcbox}的内容进行排版。如果没有给出\meta{name}，则使用已经存在的|default|盒子数组。如果使用不存在的盒子数组\meta{name}，则视为错误。如果\meta{index}超出范围，则静默忽略。可以通过|tcolorbox| \meta{options}来定制\refCom{tcbox}。
\begin{dispExample}
\boxarraygetsize{\mysize}
...
从盒子数组\meta{name}中获取给定\meta{index}号的盒子并进行排版。如果没有给出\meta{name}，则使用已经存在的|default|盒子数组。如果使用不存在的盒子数组\meta{name}，则视为错误。如果\meta{index}超出范围，则静默忽略。与\refCom{useboxarray}不同，\refCom{consumeboxarray}对应于标准的|\box|宏，即在排版盒子后，盒子寄存器被清空，不能再次使用。
\begin{dispExample}
\boxarraygetsize{\mysize}
...
使用给定的索引号从盒子数组\meta{name}中提取盒子，并使用\refCom{consumeboxarray}作为\refCom{tcbox}的内容进行排版。如果没有给出\meta{name}，则使用已存在的|default|盒子数组。如果使用了不存在的盒子数组\meta{name}，则会发生错误。如果\meta{index}超出范围，则会被静默忽略。可以通过|tcolorbox| \meta{options}自定义\refCom{tcbox}。在排版盒子后，盒子寄存器将被清空，不能再次使用。
...
\includegraphics[width=\linewidth]{/Users/virhuiai/hlProjects/Latex-Typesetting-Hub/宏包文档翻译/tcolorbox/Basilica_5.png}
\begin{center}
