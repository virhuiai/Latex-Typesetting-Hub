...
下面提供了一个指南，用于创建带有答案的 \LaTeX\ 练习。这些答案被保存到磁盘上，以便在需要的地方应用。因此，所有使用的练习都被记录在一个名为 |\jobname.records| 的文件中，以便自动处理。解答内容本身保存在一个名为 |solutions| 的子目录中。请参见 \Vref{sec:recording}。
\begin{itemize}
\item Before the first exercise is given,
\refCom{tcbstartrecording} has to be called to start recording.
\\在给出第一个练习之前，必须调用 \refCom{tcbstartrecording} 开始录制。
...
\\解决方案作为 \refEnv{tcboutputlisting} 环境的内容给出。请注意，您可以使用编译后的形式在练习中使用 \refCom{tcbuselistingtext}。
...
\\在给出最后一个练习（且在使用解决方案之前），必须调用 \refCom{tcbstoprecording} 停止录制。
\item The solutions are loaded by \refCom{tcbinputrecords}.
\\解决方案通过 \refCom{tcbinputrecords} 加载。
\end{itemize}
...
在练习文本中，可能存在需要作为\LaTeX 源代码和编译后文本的文本部分。这些部分可以通过\refEnv{tcbwritetemp}保存，并通过\refCom{tcbusetemp}以编译形式使用，或通过\refCom{tcbusetemplisting}作为源代码使用。
