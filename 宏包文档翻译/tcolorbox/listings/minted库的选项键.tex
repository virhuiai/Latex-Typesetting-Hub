...
设置使用 |minted| \cite{poore:minted} 包在列表排版期间使用的选项。另请参见 \refKeyLe{/tcb/minted options app} 和 \refKeyLe{/tcb/minted options pre}。
\begin{dispExample}
% \tcbuselibrary{skins}
...
如果未使用\refKeyLe{/tcb/minted options}，则从|minted|\cite{poore:minted}包设置选项，用于列表的排版。预期的用途是在导言中更改默认行为。请注意，设置\refKeyLe{/tcb/default minted options}也会重置\refKeyLe{/tcb/minted options}。
\begin{dispListing}
% inside the preamble
\tcbset{%
...
设置一个已知于 |Pygments| \cite{pygments:web} 的 \meta{style}。这与 \refKeyLe{/tcb/minted options} 是独立的。请注意，样式始终是全局应用的；所有后续的示例都将设置在给定的 \meta{style} 中，直到设置新样式。还请注意，如果文档中的所有样式都相同，则每个文档仅需设置一次 |\usemintedstyle|\marg{style} 更为经济。有关不同样式的示例，请参见 \refKeyLe{/tcb/minted language} 和 \refKeyLe{/tcb/minted options}。
\end{docTcbKey}
