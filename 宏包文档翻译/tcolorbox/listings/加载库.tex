...
与其他 |tcolorbox| 库不同的是，库 \mylib{listings}、\mylib{listingsutf8} 和 \mylib{minted} 在并发方面是相同的，即它们都可以以并行的方式显示带有或不带有 \LaTeX\ 排版的代码清单。它们之间的区别在于底层的 \LaTeX\ 包，该包负责显示清单。因此，通常你只需要使用其中的 \emph{一个} 库。如果你不知道应该使用哪一个库，且使用的是 |pdflatex|，那么你应该选择 \mylib{listingsutf8}。如果你使用的是 |xelatex| 或 |lualatex|，那么你应该选择 \mylib{listings}，因为 |xelatex| 和 |lualatex| 与 \mylib{listingsutf8} 不兼容。
