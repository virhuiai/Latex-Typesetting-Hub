...
默认的\refKeyLe{/tcb/listing file}设置不能用于编译源码清单，因为基本名称等于|\jobname|，包含的PDF文件应该是唯一的。 因此，要使用\refKeyLe{/tcb/run pdflatex}等，必须将\refKeyLe{/tcb/listing file}设置为唯一值。 可以使用\refKeyLe{/tcb/compilable listing}来实现这个目的。
\end{marker}
\end{docTcbKey}