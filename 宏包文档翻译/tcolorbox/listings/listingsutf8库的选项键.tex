...
使用\refKey{/tcb/listing inputencoding}的缩写，结合来自包|listingsutf8|\cite{oberdiek:listingsutf8}的UTF-8支持。此选项仅适用于库变体\mylib{listingsutf8}。\meta{one-byte-encoding}是来自\cite{oberdiek:listingsutf8}的适用编码之一，例如|latin1|是默认值。\par 请注意，这意味着限制在特定的\meta{one-byte-encoding}上：例如|latin1|包括umlauts和其他重音字符，但不包括欧元符号。如果您想同时使用|listings|包和“真正的”UTF-8源代码，则不要使用\mylib{listingsutf8}，而是使用\mylib{listings}，并使用\refKey{/tcb/listing inputencoding}|=utf8|以及针对特定UTF-8编码字符的手动修补。
\end{docTcbKey}
