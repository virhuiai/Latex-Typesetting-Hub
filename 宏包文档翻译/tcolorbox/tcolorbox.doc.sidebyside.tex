...
\begin{stripedbox}
\refCom{tcbsidebyside} is not only a shortcut for using a normal \refEnv{tcolorbox} with \refKeyLe{/tcb/sidebyside}, 
but allows setting further options like \refKeyLe{/tcb/sidebyside adapt} and \refKeyLe{/tcb/sidebyside switch}.
\tcblower
\refCom{tcbsidebyside}不只是使用普通的\refEnv{tcolorbox}指定\refKeyLe{/tcb/sidebyside}的快捷方式， 
