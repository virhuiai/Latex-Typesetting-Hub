...
一个\emph{并排}的盒子是一个特殊的\refEnvLe{tcolorbox}，其中盒子的上部和下部并排设置。这种类型的所有盒子都是\csh渐变盒子{不可分割的}。
\end{document}
...
\tcblower
正常情况下，盒子的upper部分和lower部分的位置，同它们的名字一致。如果 sidebyside 设置为 true, 则upper部分会放置在左边，lower部分放置在右边。两部分的外观绘制设置使用的upper部分的设置，但水平分割的设置是根据以下选项。颜色、字体和盒子的内容添加可分别设置。最终的盒子是\csh渐变盒子V[red]{不可分页}的。
\end{stripedbox}
% 这两个部分与upper部分的几何设置一起绘制，但是空间根据以下选项水平分割。
