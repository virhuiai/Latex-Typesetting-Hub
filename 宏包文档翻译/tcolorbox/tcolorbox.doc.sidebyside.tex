...
\tcblower
\emph{side by side} 盒子是一个特殊的 \refEnv{tcolorbox} 环境，此种
盒子的上部和下部水平并排放置.
所有这种类型的盒子是不可分页的。
\end{stripedbox}
...
\refKey{/tcb/listing outside text}, 和
...
The \refKey{/tcb/sidebyside} option is set to |true| and the \meta{left-handed content} and \meta{right-handed content} is filled into the box appropriately.
The resulting box is unbreakable.
\tcblower
为\refEnv{tcolorbox}指定任意的选项\meta{options}创建一个 |tcolorbox| 盒子。%
\refKey{/tcb/sidebyside}选项被设置为|true|， \meta{left-handed content}和\meta{right-handed content}被适当地填充到盒子中。%
这样的盒子是不可分页的。
\end{stripedbox}

\begin{stripedbox}
\refCom{tcbsidebyside} is not only a shortcut for using a normal \refEnv{tcolorbox} with \refKey{/tcb/sidebyside}, 
but allows setting further options like \refKey{/tcb/sidebyside adapt} and \refKey{/tcb/sidebyside switch}.
\tcblower
\refCom{tcbsidebyside}不只是使用普通的\refEnv{tcolorbox}指定\refKey{/tcb/sidebyside}的快捷方式， 
还允许设置更多的选项，如\refKey{/tcb/sidebyside adapt}和\refKey{/tcb/sidebyside switch}。
\end{stripedbox}
...
This option is only valid inside \refCom{tcbsidebyside}.
\tcblower
该选项允许根据左边和/右边内容确定盒子的尺寸。
此选项仅在\refCom{tcbsidebyside}内有效。
\end{stripedbox}
...
\tcblower
左侧内容的实际宽度用于设置 \refKey{/tcb/lefthand width}。
\end{stripedbox}
    \item\docValue{right}:
\begin{stripedbox}
the actual width of the right-handed content is used to set \refKey{/tcb/righthand width}.
\tcblower
右侧内容的实际宽度用于设置\refKey{/tcb/右手宽度}。
\end{stripedbox}

    \item\docValue{both}:
\begin{stripedbox}
the actual width of the left-handed and right-handed content is used to set \refKey{/tcb/lefthand width},  \refKey{/tcb/righthand width}, and the overall \refKey{/tcb/width}.
\tcblower
左手和右手内容的实际宽度用于设置 \refKey{/tcb/lefthand width} 和 \refKey{/tcb/righthand width}，%
和整个\refKey{/tcb/width}。
\end{stripedbox}
...
\tcblower
如果设置为|true|,%
\refCom{tcbsidebyside}的 \meta{left-handed content} 和 \meta{right-handed content} 会被切换。%
显然，这个选项只在\refCom{tcbsidebyside}内有效。
\end{stripedbox}

\begin{stripedbox}
The side switching can be made even/odd page sensitive, 
if used inside \refKey{/tcb/if odd page}.
\tcblower
如果指定了了 \refKey{/tcb/if odd page}，两侧切换对奇/偶页敏感。
\end{stripedbox}
