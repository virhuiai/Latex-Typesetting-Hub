...
the dimensions are made smaller and \refKey{/tcb/nobeforeafter} and \refKey{/tcb/tcbox raise base} are set.
\tcblower
设置\refCom{tcbox}拥有\textit{verbatim}样式的选项。%
由于缩进的盒子可能只包含很少的单词，%
尺寸被设置变小，还指定\refKey{/tcb/nobeforeafter}和\refKey{/tcb/tcbox raise base}选项。
\end{stripedbox}
...
\tcblower
基于\refEnv{tcolorbox}创建一个新的环境\meta{name}。
\end{stripedbox}
...
\meta{options}被赋予底层的\refEnv{tcolorbox}。
\end{stripedbox}
...
automatically.
\tcblower
注意，\refKey{/tcb/savedelimiter}自动被设置为给定的\meta{name}。
\end{stripedbox}
...
\begin{stripedbox}
Operates like \refCom{DeclareTColorBox}, 
...
\tcblower
操作方式类似\refCom{DeclareTColorBox}，%
...
\tcblower
操作类似\refCom{DeclareTColorBox}，但基于|\ProvideDocumentEnvironment|而不是|\DeclareDocumentEnvironment|。
...
\tcblower
基于\refEnv{tcolorbox}创建一个新命令\texttt{\textbackslash}\meta{name}。%
与\refCom{DeclareTColorBox}相反，|tcolorbox|的\meta{content}也被指定。
\end{stripedbox}
...
\meta{options}被设置到底层的\refEnv{tcolorbox}，该\refEnv{tcolorbox}被指定的\meta{content}填充。
\end{stripedbox}
...
\tcblower
操作类似\refCom{DeclareTotalTColorBox}，但基于|\NewDocumentCommand|而不是|\DeclareDocumentCommand|
...
\tcblower
操作类似\refCom{DeclareTotalTColorBox}，但基于|\RenewDocumentCommand|而不是|\DeclareDocumentCommand|。
重新定义现有命令。
\end{stripedbox}
\end{docCommand}
...
\tcblower
操作类似\refCom{DeclareTotalTColorBox}，但基于|\ProvideDocumentCommand|而不是|\DeclareDocumentCommand|。命令\texttt{\textbackslash}\meta{name}只有在尚未定义时才会被创建。
\end{stripedbox}  
\end{docCommand}
...
\tcblower
基于\refCom{tcbox}创建一个新命令\texttt{\textbackslash}\meta{name}。%
基本上，|\DeclareTCBox|操作类似|\DeclareDocumentCommand|。这意味着,%
新命令\texttt{\textbackslash}\meta{name}是用给定的参数\meta{specification}构造的。\meta{options}被赋给底层的\refCom{tcbox}。
\end{stripedbox}
...
An error is issued if \texttt{\textbackslash}\meta{name} has already been defined.
\tcblower
操作类似\refCom{DeclareTCBox}，但基于|\NewDocumentCommand|而不是|\DeclareDocumentCommand|。
如果已经定义了\texttt{\textbackslash}\meta{name}，则会发出错误。
\end{stripedbox}
\end{docCommand}
...
An existing command is redefined.
\tcblower
操作类似\refCom{DeclareTCBox}，但基于|\RenewDocumentCommand|而不是|\DeclareDocumentCommand|。%
重新定义现有命令。
\end{stripedbox}
\end{docCommand}
...
The command \texttt{\textbackslash}\meta{name} is only created if it is not already defined.
\tcblower
操作类似\refCom{DeclareTCBox}，但基于|\ProvideDocumentCommand|而不是|\DeclareDocumentCommand|。
...
\tcblower
基于\refCom{tcbox}创建一个新命令\texttt{\textbackslash}\meta{name}。%
与\refCom{DeclareTCBox}相反，|tcbox|的\meta{content}也被指定。
\end{stripedbox}
...
\meta{options}被赋予底层的\refCom{tcbox}，该\refCom{tcbox}被填充
指定的\meta{content}。
\end{stripedbox}
...
automatically.
\tcblower
注意\refKey{/tcb/savedelimiter}被自动设置为给定的\meta{name}。
\end{stripedbox}
...
An error is issued if \texttt{\textbackslash}\meta{name} has already been defined.
\tcblower
操作类似\refCom{DeclareTotalTCBox}，但基于|\NewDocumentCommand|而不是|\DeclareDocumentCommand|。%
如果已经定义了\texttt{\textbackslash}\meta{name}，则会发出错误。
\end{stripedbox}
\end{docCommand}
...
An existing command is redefined.
\tcblower
操作类似\refCom{DeclareTotalTCBox}，但基于|\RenewDocumentCommand|而不是|\DeclareDocumentCommand|。重新定义现有命令。
\end{stripedbox}
\end{docCommand}
...
The command \texttt{\textbackslash}\meta{name} is only created if it is not already defined.
\tcblower
操作类似\refCom{DeclareTotalTCBox}，但基于|\ProvideDocumentCommand|而不是|\DeclareDocumentCommand|。命令\texttt{\textbackslash}\meta{name}只有在尚未定义时才会被创建。
\end{stripedbox}
\end{docCommand}
...
\tcblower
基于\refCom{tcbox}创建一个与给定\meta{verbatim box content}宽度相匹配的彩色盒子。
底层的\refCom{tcbox}的样式使用\refKey{/tcb/verbatim}加上给定的\meta{options}进行设置。
\refCom{tcbox}的不同之处在于\meta{verbatim box content}被\textit{逐个输出}。因此，|\tcboxverb|的行为类似于|\verb|。
\end{stripedbox}
...
\tcblower
基于\refEnv{tcblisting}创建一个新环境\meta{name}。
\end{stripedbox}
...
新的环境\meta{name}是用给定的参数\meta{specification}构造的。
\meta{options}被设置到底层的\refEnv{tcblisting}。
\end{stripedbox}
...
An error is issued if \meta{name} has already been defined.
\tcblower
操作类似\refCom{DeclareTCBListing}，但基于|\NewDocumentEnvironment|而不是|\DeclareDocumentEnvironment|。
如果已经定义了\meta{name}，则会发出错误。
\end{stripedbox}
\end{docCommand}
...
An existing environment is redefined.
\tcblower
操作类似\refCom{DeclareTCBListing}，但基于|\RenewDocumentEnvironment|而不是|\DeclareDocumentEnvironment|。
重新定义现有环境。
\end{stripedbox}
\end{docCommand}
...
The environment \meta{name} is only created if it is not already defined.
\tcblower
操作类似\refCom{DeclareTCBListing}，但基于|\ProvideDocumentEnvironment|而不是|\DeclareDocumentEnvironment|。
环境\meta{name}只有在尚未定义时才会被创建。
\end{stripedbox}    
\end{docCommand}
...
command with the same name.
\tcblower
基于\refCom{tcbinputlisting}创建一个新命令\texttt{\textbackslash}\meta{name}。%
...
\meta{options}被赋给底层的\refCom{tcbinputlisting}。\\%
\meta{init选项}允许设置自动编号，
请参见 \pageref{sec:initkeys} 页的 \ref{sec:initkeys} 章节。\\
...
An error is issued if \texttt{\textbackslash}\meta{name} has already been defined.
\tcblower
操作类似\refCom{DeclareTCBInputListing}，但基于|\NewDocumentCommand|而不是|\DeclareDocumentCommand|。
如果已经定义了\texttt{\textbackslash}\meta{name}，则会发出错误。
\end{stripedbox}
\end{docCommand}
...
An existing command is redefined.
\tcblower
操作类似\refCom{DeclareTCBInputListing}，但基于|\RenewDocumentCommand|而不是|\DeclareDocumentCommand|。
重新定义现有命令。
\end{stripedbox}
\end{docCommand}
...
The command \texttt{\textbackslash}\meta{name} is only created if it is not already defined.
\tcblower
作类似\refCom{DeclareTCBInputListing}，但基于|\ProvideDocumentCommand|而不是|\DeclareDocumentCommand|。
命令\texttt{\textbackslash}\meta{name}只有在尚未定义时才会被创建。
\end{stripedbox}
\end{docCommand} 
...
An error is issued if \texttt{\textbackslash}\meta{name} has already been defined.
\tcblower
操作类似\refCom{DeclareTCBoxFit}，但基于|\NewDocumentCommand|而不是|\DeclareDocumentCommand|。
如果已经定义了\texttt{\textbackslash}\meta{name}，则会发出错误。
\end{stripedbox}
\end{docCommand}
...
An existing command is redefined.
\tcblower
操作类似\refCom{DeclareTCBoxFit}，但基于|\RenewDocumentCommand|而不是|\DeclareDocumentCommand|。
重新定义现有命令。
\end{stripedbox}
\end{docCommand}
...
The command \texttt{\textbackslash}\meta{name} is only created if it is not already defined.
\tcblower
操作类似\refCom{DeclareTCBoxFit}，但基于|\ProvideDocumentCommand|而不是|\DeclareDocumentCommand|。
命令\texttt{\textbackslash}\meta{name}只有在尚未定义时才会被创建。
\end{stripedbox}
\end{docCommand}
...
\tcblower
基于\refCom{tcboxfit}创建一个新命令\texttt{\textbackslash}\meta{name}。
与\refCom{DeclareTCBoxFit}不同，|tcboxfit|的\meta{content}也被指定。\\
基本上，|\DeclareTotalTCBoxFit|操作类似|\DeclareDocumentCommand|。这意味着,
新命令\texttt{\textbackslash}\meta{name}是用给定的参数\meta{specification}构造的。
\meta{options}被设置到底层的\refCom{tcboxfit}，该\refCom{tcboxfit}被填充指定的\meta{content}。\\
注意\refKey{/tcb/savedelimiter}被自动设置为给定的\meta{name}。\\
\meta{init选项}允许设置自动编号，
请参见 \pageref{sec:initkeys} 页的 \ref{sec:initkeys} 章节。\\
新命令总是被创建，不管是否已经存在同名命令。
...
An error is issued if \texttt{\textbackslash}\meta{name} has already been defined.
\tcblower
操作类似\refCom{DeclareTotalTCBoxFit}，但基于|\NewDocumentCommand|而不是|\DeclareDocumentCommand|。
如果已经定义了\texttt{\textbackslash}\meta{name}，则会发出错误。
\end{stripedbox}
\end{docCommand}
...
An existing command is redefined.
\tcblower
操作类似\refCom{DeclareTotalTCBoxFit}，但基于|\RenewDocumentCommand|而不是|\DeclareDocumentCommand|。
重新定义现有命令。
\end{stripedbox}
\end{docCommand}
...
The command \texttt{\textbackslash}\meta{name} is only created if it is not already defined.
\tcblower
操作类似\refCom{DeclareTotalTCBoxFit}，但基于|\ProvideDocumentCommand|而不是|\DeclareDocumentCommand|。
命令\texttt{\textbackslash}\meta{name}只有在尚未定义时才会被创建。
\end{stripedbox}
\end{docCommand}