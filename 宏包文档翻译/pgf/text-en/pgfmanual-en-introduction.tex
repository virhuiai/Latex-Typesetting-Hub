...
% 既然我们知道了什么是 \tikzname\，那么“\pgfname”又是什么呢？如前所述，\tikzname\ 开始是一个项目，用于实现可以与 pdf\LaTeX\ 和传统的（基于 PostScript 的）\LaTeX\ 一起使用的 \TeX\ 图形宏。换句话说，我想要实现一个 \TeX\ 的“便携式图形格式” —— 因此得名 \pgfname。这些早期的宏仍然存在，它们构成了本手册中描述的系统的“基础层”，但是这些天作者的大部分交互都是与 \tikzname\ —— 它已经成为了一个完整的语言。
