...
\end{codeexample}
    ...
    在本节的其余部分将详细描述\meta{animation specification}中确切发生的事情。基本上，\meta{animation specification}只是一系列正常的\tikzname 键值对，它们使用路径前缀 |/tikz/animate| 执行，并安装了一些特殊的语法处理程序。特别地，您可以为此键路径定义样式并使用它们。例如，我们可以像这样定义一个|shake|动画：
