...
\switchcolumn
默认情况下，使用 \pkg{float} 宏包来创建 |listing| 环境。在这种情况下，可以使用下面描述的 |\listingscaption| 和 |\listoflistingscaption| 宏来自定义标题和清单列表。如果加载了带有 |newfloat| 选项的 \pkg{minted} 宏包，则 |listing| 环境将使用功能更强大的 \href{http://www.ctan.org/pkg/newfloat}{\pkg{newfloat}} 宏包创建。 \pkg{newfloat} 是 \href{http://www.ctan.org/pkg/caption}{\pkg{caption}} 的一部分，它提供了许多自定义标题的选项。
\switchcolumn[0]*%%%%%%%%%%%%
...
\end{verbatim}
\switchcolumn
当使用 \pkg{newfloat} 来创建 |listing| 环境时，应使用 \pkg{newfloat} 的 |\SetupFloatingEnvironment| 命令来实现自定义。例如，可以使用以下方式将标题中的字符串“Listing”更改为“程序代码”：
\begin{verbatim}
...
\switchcolumn
请参考 \pkg{newfloat} 和 \pkg{caption} 文档以获取更多信息。
\switchcolumn[0]*%%%%%%%%%%%%
...
\switchcolumn
\mint[frame=lines]{latex}/\renewcommand{\listingscaption}{程序代码}/
\switchcolumn[0]*%%%%%%%%%%%%
...
\switchcolumn
\mint[frame=lines]{latex}/\renewcommand{\listoflistingscaption}{程序代码列表}/
\end{paracol}